...
function Screen (hardware, options, callback) {
  ...
  this.spi = this.hardware.SPI({clockSpeed : 8 * 1000 * 1000, mode:0, chipSelect:this.chipSelect});
  ...
  this._sendCommand('init', function (err, packet) {
    packet.readStream.on('data', function (data){
      if (data.command == lib.commands.ILI9341_DISPON) {
        this._writeToSpi(data, function () {
          this.emit('ready');
        }.bind(this));
      } else {
        this._writeToSpi(data);
      }
    }.bind(this))
  }.bind(this));
}
...
Screen.prototype._sendCommand = function (apiCommand, args, callback) {
  ...
  this._lib.getCommandPacket(apiCommand, args, function (err, packet) {
    if (err) {
...
Screen.prototype._writeToSpi = function (data, callback) {
  var commandBuf = new Buffer([data.command]);
  this.dc.output(false);
  this.spi.transferBatch(commandBuf, {start:0, chunking:1}, function (err, rx) {
    this.dc.output(true);
    var dataBuf = new Buffer(data.args)
    this.spi.transferBatch(dataBuf, {start:0, chunking:1}, function (err, rx) {
      if (callback) {
        callback();
      }
    }.bind(this))
